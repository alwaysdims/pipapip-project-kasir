...
<div class="content">
    <div id="header-footer-modal" class="p-5">
        <div class="preview">
            <!-- BEGIN: Modal Toggle -->
            <div class="text-left"> 
                <a href="javascript:;" data-tw-toggle="modal" data-tw-target="#insert-modal" class="btn btn-primary">Tambah Pengeluaran</a> 
            </div>
            <!-- END: Modal Toggle -->
                    ...
                    <div class="modal-content">
                        ...
                        <form action="<?= base_url('pengeluaran/store') ?>" method="post">
                            <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
                                ...
                                <div class="col-span-12">
                                    <label for="keterangan-insert" class="form-label">Keterangan</label>
                                    <input id="keterangan-insert" type="text" class="form-control" name="keterangan" 
                                           placeholder="Contoh: Bayar listrik bulan ini, Gaji karyawan Januari, Beli ATK" required>
                                </div>
                                <div class="col-span-12">
                                    <label for="jumlah-insert" class="form-label">Jumlah (Rp)</label>
                                    <input id="jumlah-insert" type="number" class="form-control" name="jumlah" 
        ...
        <table id="example" class="stripe" style="width:100%">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="whitespace-nowrap"><?= date('D, d M Y', strtotime($data->tanggal)) ?></td>
                    <td class="whitespace-nowrap"><?= htmlspecialchars($data->nama_tipe) ?></td>
                    <td class="whitespace-nowrap"><?= htmlspecialchars($data->keterangan) ?></td>
                    <td class="whitespace-nowrap">Rp <?= number_format($data->jumlah, 0, ',', '.') ?></td>
    ...
    <!-- Edit & Delete Modals -->
    <?php foreach($pengeluarans as $data){ ?>
    <!-- Edit Modal -->
    <div id="edit-modal-<?= $data->id ?>" class="modal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h2 class="font-medium text-base mr-auto">Edit Pengeluaran</h2>
                </div>
                <form action="<?= base_url('pengeluaran/edit/' . $data->id) ?>" method="post">
                    <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
                        <div class="col-span-12">
                            <label class="form-label">Tipe Pengeluaran</label>
                            <select name="tipe_id" class="form-select" required>
                                <option value="">Pilih Tipe</option>
                                <?php foreach($tipes as $tipe): ?>
                                <option value="<?= $tipe->id ?>" <?= $tipe->id == $data->tipe_id ? 'selected' : '' ?>>
    ...
    <div id="delete-confirmation-modal-<?= $data->id ?>" class="modal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="<?= base_url('pengeluaran/delete/' . $data->id) ?>" method="post">
                    <div class="modal-body">
                        <p>Apakah kamu yakin ingin menghapus pengeluaran "<strong><?= htmlspecialchars($data->keterangan) ?></strong>" sebesar <strong>Rp <?= number_format($data->jumlah, 0, ',', '.') ?></strong>?</p>
                    </div>
                    <div class="modal-footer">
                        <button type="button" data-tw-dismiss="modal" class="btn btn-outline-secondary w-20 mr-1">Cancel</button>
